...
run_model <- function(dt, run_time, state_init, parameters, seed2, scenario, sys_time, output_folder) {
    ...
    plot_states <- output %>% 
        ...
        mutate(grid = if_else(compartment %in% c("S", "V", "R1", "R2"), "A", "B")) %>%
        filter(time %in% seq(0, run_time, 1)) %>% 
        ...
        geom_vline(xintercept = seed2$time[1],
                   linetype="dashed",
                   size=0.5)
    
    plot_name <- file.path(".", output_folder, sprintf("%s_%s_plot.png", scenario, sys_time))
