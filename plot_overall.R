...
resident_preinf <- unique(end$preinf[end$seed==resident_seedproxy])
...
variant_label <- c("Resident strain only" = "Resident strain only",
                   "Pre-infectious (d[E]) 0.5 days" = bquote(d[E] ~ 0.5 ~ days),
                   "More transmissable (r[init] matched to d[E] 0.5)" = bquote(Higher ~ R[0] ~ (r[init] ~ matched ~ to ~ d[E]~0.5)),
                   "Pre-infectious (d[E]) 1.5 days" = bquote(d[E] ~ 1.5 ~ days),
                   "More transmissable (r[init] matched to d[E] 1.5)" = bquote(Higher ~ R[0] ~ (r[init] ~ matched ~ to ~ d[E]~1.5)))
...
more_transmiss_r1pt5 <- matched_r0 %>% 
    ...
    labs(title = bquote("Resident strain" ~ R[0] == 2.5 ~ ";" ~ d[E] == .(resident_preinf) ~ days),
         x = bquote(Shorter~d[E]~"variant matched to"),
...
ggsave(file.path(".", output_folder, "more_transmiss_r0s_r2.5.png"), width = 20, height = 7, units = "cm", dpi = 300)
...
more_transmiss_r4 <- matched_r0 %>% 
    ...
    labs(title = bquote("Resident strain" ~ R[0] == 4 ~ ";" ~ d[E] == .(resident_preinf) ~ days),
         x = bquote(Shorter~d[E]~"variant matched to"),
         y = bquote("More transmissible variant" ~~ R[0]),
         shape = "Cross-immunity",
         colour = "Variant seed day") +
...
more_transmiss_r1pt5 / more_transmiss_r4 + plot_layout(guides = 'collect')

ggsave(file.path(".", output_folder, "more_transmiss_r0s_r1.5_r4.png"), width = 20, height = 14, units = "cm", dpi = 300)


#### Total size ------
# graph_r0 <- 2.5
graph_vaccstart_range <- sort(unique(end$vacc_start[end$r0_strain1==graph_r0]))
graph_caption <- bquote("Resident strain" ~ R[0] == .(graph_r0) ~ "; pre-infectious period" ~ (d[E]) == .(resident_preinf) ~ days)
...
## Cumulative new infections, relative to population size
graph_table %>%
...
graph_name <- file.path(".", output_folder, sprintf("cuminf_r%s.png", graph_r0))
...
graph_name <- file.path(".", output_folder, sprintf("reinfections_r%s.png", graph_r0))
...
graph_name <- file.path(".", output_folder, sprintf("strain2_only_r%s.png", graph_r0))
...
graph_name <- file.path(".", output_folder, sprintf("strain1_only_r%s.png", graph_r0))
...
graph_name <- file.path(".", output_folder, sprintf("propinf_vacc_r%s.png", graph_r0))
...
graph_name <- file.path(".", output_folder, sprintf("peak_new_I1_I2_r%s.png", graph_r0))
...
graph_table %>%
    ...
    filter(!(strain=="Novel variant" & seed==resident_seedproxy)) %>% 
    ggplot(aes(x=seed_dodge, y=count, colour=label, group=crossimm_perc, shape=crossimm_perc)) +
    geom_point() +
    geom_line(aes(group=line_group, linetype=crossimm_perc),
              size = 0.5,
...
graph_name <- file.path(".", output_folder, sprintf("peak_time_I1_I2_r%s.png", graph_r0))
...
graph_name <- file.path(".", output_folder, sprintf("proppop_infectingstrain_r%s.png", graph_r0))
...
# for (i in 1:length(graph_vacc_label)) {
    ...
    graph_name <- file.path(".", output_folder, sprintf("newinfI2_I1_r%s_vacccov%s.png", graph_r0, str_extract(graph_vacc_label[i], "\\d+\\.?\\d*")))
...
graph_name <- file.path(".", output_folder, sprintf("newinfI1_I2_r%s_log_grid.png", graph_r0))
