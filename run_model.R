...
transmiss_range <- seq(1, 2, 0.2)
crossimm_range <- seq(0.2, 1, 0.2)
seed_range <- seq(0, 140, 14)
vacc_range <- seq(0, 0.8, 0.05)
crossing(transmiss_range, crossimm_range, seed_range, vacc_range)

# Unique scenario name
scenario <- sprintf("transmiss%s_crossimm%s_seed%s_vacc%s", )
...
strain1 <- c(
    ...
    r0 = 3,
    crossimm = 1,
    vacceff_i = 0.9*vaccination[["yes"]],
    vacceff_d = 0.6*vaccination[["yes"]]
)
